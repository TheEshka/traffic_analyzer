...
DATE_INTERVAL = 60
#Список конечных параметров для сравнения потоков. В коде помечены теми же числами для ориентирования
FEATURES = [
	'client_package_size_mean', #1 Средний размер пакета от клиента
	'client_package_size_std', #2 Среднеквадратическое отклонение размера пакета от клиента
	...
	'client_batch_sizes_mean', #5 Средний размер партии данных от клиента
	...
	'client_efficiency', #11 Отношение полезной нагрузки ко всей клиента
	'server_efficiency', #12 Отношение полезной нагрузки ко всей сервера
	'ratio_sizes', #13 Отношения общих размеров пакетов клиента к серверу
	'ratio_application_size', #14 Отношения суммы полезных нагрузок клиента к серверу
	'ratio_packages', #15 Отношения количества пакетов клиента к серверу
	'client_package_size_sum', #16 Сумма размеров пакетов клиента
	'client_application_size_sum', #17 Сумма полезной нагрузки пакетов клиента
	'client_package_count', #18 Количества пакетов клиента
	'client_batch_counts_sum', #19 Количество партий клиента
	'server_package_size_sum', #20 Сумма размеров пакетов сервера
	'server_application_size_sum', #21 Сумма полезной нагрузки пакетов сервера
	'server_package_count', #22 Количества пакетов сервера
	'server_batch_counts_sum', #23 Количество партий сервера
	'transport_protocol', #24 Номер протокола транспортного уровня
	'ip_protocol_version', #25 Версия IP протокола
]
...
def read_pcap_and_return_dataframe(filePath):
	...
	# traffic = pd.read_csv('./file.csv', encoding = "ISO-8859-1", low_memory=False)
